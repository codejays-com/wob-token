...
/**
    Location now store Drop Contract Address
**/
...
contract WorldOfBlastLocation is ERC721URIStorage, Ownable {
    ...
    address private dropContractAddress; // Variable to store the drop contract address
    ...
    event DropContractAddressUpdated(address indexed newAddress);
    ...
    function setDropContractAddress(address _address) external onlyOwner {
        require(_address != address(0), "Invalid address");
        dropContractAddress = _address;
        emit DropContractAddressUpdated(_address);
    }

    function getDropContractAddress() external view returns (address) {
        return dropContractAddress;
    }
